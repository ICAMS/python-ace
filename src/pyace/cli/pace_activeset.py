...
def main():
    ...
    try:
        ...
        log.info("ACEBBasisSet loaded")
    except RuntimeError:
        bbasis = GRACEFSBasisSet(potential_file)
        log.info("GRACEFSBasisSet loaded")
    ...
    if batch_size is None:
        ...
        if not all_atoms:
            ...
            for symb, st in elements_to_index_map.items():
                ...
                dforces_comp = np.linalg.norm(dforces, axis=1)/np.sqrt(3)

                if error_based_threshold <= 0:
                    q1, q2, q3 = np.quantile(dforces_comp, q=[0.25, 0.5, 0.75], )
                    iqr = q3 - q1
                    upper_bound = q3 + 1.5 * iqr
                else:
                    upper_bound = error_based_threshold

                sel_mask = dforces_comp <= upper_bound
                sel_mask_dict[st] = sel_mask

                f_rmse_before = np.sqrt(np.mean(dforces_comp ** 2))

                f_rmse_after = np.sqrt(np.mean(dforces_comp[sel_mask] ** 2))
